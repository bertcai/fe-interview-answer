...
1. 通过Axios拦截器，对本地请求进行拦截。
2. 本地请求发送需要mock的数据时，添加_mock传参，值为mock的地址。
3. Axios拦截请求的返回值后，将其mock地址和mock数据匹配返回需要mock的数据。
...
export const mockTags: Mock = (config) => {
    const creatTags = (n = 1, attrs?: any) => Array.from({length: n}).map(
        (_, index) => ({
            id: index,
